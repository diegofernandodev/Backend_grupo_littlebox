...
const guardarIngreso = async(ingreso, tenantId) => {
    ...
    if (!ingreso || !ingreso.detalle || !ingreso.valor) {
        throw new Error("El valor ingreso no es valido o no contiene campos requeridos");
    }
...
const modificarIngresoPorId = async(ingresoId, nuevosDatos, tenantId) => {
    ...
    try {
        ...
        const ingresoModificado = await Ingreso.findOneAndUpdate({ _id: ingresoId, tenantId },
